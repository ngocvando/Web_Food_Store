...
const orderController = {
    ...
    postOrder: async (req, res) => {
        ...
        try {
            ...
            for (const item of cart) {
                await conn.execute(
                    'INSERT INTO order_items (order_id, dish_id, quantity, price_at_time, combo_id, is_combo, item_name_snapshot, image_snapshot) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                    [
                        orderId,
                        item.is_combo ? null : item.dish_id,
                        item.quantity,
                        item.price,
                        item.is_combo ? item.combo_id : null,
                        item.is_combo ? 1 : 0,
                        item.dish_name || item.combo_name || item.item_name || 'Sản phẩm',
                        item.image_url || null
                    ]
                );
            }
    ...
    printInvoice: async (req, res) => {
        try {
            await ensureOrderReviewSchema();
            const data = await getInvoiceData(req.params.id, req.session.userId);
            ...
            console.error(error);
            res.status(500).send('Lỗi xem hóa đơn');
        }
    },
