...
const adminController = {
    ...
    getCombos: async (req, res) => {
        try {
            ...
            res.render('admin/pages/combos', { combos, dishes, query: req.query });
        } catch (error) {
    ...
    postAddCombo: async (req, res) => {
        try {
            ...
            if (ids.length === 0) {
                return res.status(400).send('Combo phải có ít nhất 1 món ăn');
            }
    ...
    getEditCombo: async (req, res) => {
        try {
            const comboId = req.params.id;
            const [[combo]] = await db.execute('SELECT * FROM combos WHERE combo_id = ?', [comboId]);
            if (!combo) return res.status(404).send('Không tìm thấy combo');

            const [dishes] = await db.execute('SELECT * FROM dishes WHERE is_available = 1 ORDER BY dish_name');
            const [items] = await db.execute(`
                SELECT ci.dish_id, ci.quantity, d.dish_name, d.price
                FROM combo_items ci
                JOIN dishes d ON ci.dish_id = d.dish_id
                WHERE ci.combo_id = ?
            ...
            res.render('admin/pages/edit-combo', { combo, dishes, items });
        } catch (error) {
            console.error(error);
            res.status(500).send('Lỗi tải form sửa combo');
        }
    },

    postEditCombo: async (req, res) => {
        const conn = await db.getConnection();
        try {
            const comboId = req.params.id;
            const { combo_name, description, discount_price, dish_ids, quantities, is_available } = req.body;
            const ids = (Array.isArray(dish_ids) ? dish_ids : [dish_ids]).filter(Boolean);
            const qtys = Array.isArray(quantities) ? quantities : [quantities];
            ...
            let originalPrice = 0;
            for (let i = 0; i < ids.length; i++) {
                const [[dish]] = await conn.execute('SELECT price FROM dishes WHERE dish_id = ?', [ids[i]]);
                originalPrice += Number(dish?.price || 0) * Number(qtys[i] || 1);
            }
            ...
            await conn.execute(
                `UPDATE combos
                 SET combo_name = ?, description = ?, discount_price = ?, original_price = ?, is_available = ? ${imageSql}
                 WHERE combo_id = ?`,
                params
            );

            await conn.execute('DELETE FROM combo_items WHERE combo_id = ?', [comboId]);
            for (let i = 0; i < ids.length; i++) {
                await conn.execute(
                    'INSERT INTO combo_items (combo_id, dish_id, quantity) VALUES (?, ?, ?)',
                    [comboId, ids[i], Number(qtys[i] || 1)]
                );
            }

            await conn.commit();
            res.redirect('/admin/combos?updated=1');
        } catch (error) {
            await conn.rollback();
            console.error(error);
    ...
    toggleCombo: async (req, res) => {
        try {
            await db.execute('UPDATE combos SET is_available = CASE WHEN is_available = 1 THEN 0 ELSE 1 END WHERE combo_id = ?', [req.params.id]);
            res.redirect('/admin/combos');
        } catch (error) {
            console.error(error);
            res.status(500).send('Lỗi khi cập nhật trạng thái combo');
        }
    },
    ...
    deleteCombo: async (req, res) => {
        ...
        try {
            ...
            const [[used]] = await conn.execute(
                'SELECT COUNT(*) AS total FROM order_items WHERE combo_id = ?',
                [comboId]
            );

            if (Number(used.total) > 0) {
                return res.redirect('/admin/combos?error=combo_used');
            }

            await conn.beginTransaction();
            await conn.execute('DELETE FROM combo_items WHERE combo_id = ?', [comboId]);
            await conn.execute('DELETE FROM combos WHERE combo_id = ?', [comboId]);
            await conn.commit();

            res.redirect('/admin/combos?deleted=1');
        } catch (error) {
    ...
    postAddVoucher: async (req, res) => {
        try {
            ...
            const minOrderAmount = toNonNegativeNumber(min_order_amount);
            const maxDiscountAmount = toNonNegativeNumber(max_discount_amount);
            const usageLimitValue = Math.floor(toNonNegativeNumber(usage_limit));
            ...
            // Nếu voucher đã bị xóa mềm thì khôi phục lại bản ghi cũ thay vì INSERT bản ghi mới,
            // tránh lỗi UNIQUE KEY trên cột code khi admin thêm lại cùng một mã voucher.
            const [existingVouchers] = await db.execute(
                `SELECT discount_id, deleted_at
                 FROM discount_codes
                 WHERE code = ?
                 LIMIT 1`,
                [normalizedCode]
            ...
            if (existingVouchers.length > 0) {
                ...
                if (existingVoucher.deleted_at) {
                    ...
                    `, [
                        ...
                        startDateValue,
                        endDateValue,
                        existingVoucher.discount_id
                    ]);

                    return res.redirect('/admin/vouchers?restored=1');
                }

                return res.redirect('/admin/vouchers?error=duplicate_code');
            }
